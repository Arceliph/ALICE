...
def resize_image_bytes(img_buffer, target_filesize, tolerance=5):
...
async def furina_api(ctx, id_num=None):
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(url) as furina_request:
            ...
            async with session.get(furina_img_loc) as furina_image_request:
                ...
                if len(furina_image_data.getvalue()) >= DISCORD_SIZE_LIMIT_BYTES:
